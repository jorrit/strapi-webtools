...
const customServices = () => ({
  ...
  makeUniquePath: async (
    ...
    const test = await strapi.documents('plugin::webtools.url-alias').findFirst({
    });
    console.log(test);
    ...
    for (let iteration = -1; ; ++iteration) {
      ...
      console.log(`existing for ${newPath} = `, existingPathEntity);
    ...
    console.log(`[${currentDocumentId},${currentLocale}] ${originalPath} -> ${newPath}`);
    return newPath;
  },
});
